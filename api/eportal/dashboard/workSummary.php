...
if ((int)$row['LWP_DAYS'] > 0) {
    $data['summary'] = ["category" => "LWP", "value" => $row['LWP_DAYS']];
}
...
$avgWorkHrs = singRec("SELECT round(avg(work_hour) , 2 ) HRS FROM  ept_bcs_attd_reg WHERE emp_code='".$empCode."'");
$data['avgWorkHrs'] = $avgWorkHrs['0'];
