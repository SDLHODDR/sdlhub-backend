...
function processDeclarationDownload(
    ...
    */
    if ($type === "single") {
		...
		if (!$zip) {
			responseError("ZIP object not available.");
		}
...
/*
=====================================================
CREATE DOCUMENT DOWNLOAD JOB
=====================================================
*/
function createDocumentDownloadJob(
    $con,
    string $financialYear,
    string $requestedBy,
    string $type = "all"
    ...
    oci_bind_by_name($stmt, ":download_type", $type);
    ...
    if (!oci_execute($stmt, OCI_NO_AUTO_COMMIT)) {
        $e = oci_error($stmt);
        oci_rollback($con);
        responseError("Unable to create document job : " . $e['message']);
    }
...
function startDocumentWorker(
    ...
    $workerFile = __DIR__ . "/../jobs/processDocumentsDownload.php";
    ...
    $workerLog = ITR_JOB_LOG_PATH . "/document_worker.log";
...
/*
=====================================================
CREATE DECLARATION DOWNLOAD JOB
=====================================================
*/
function createDeclarationDownloadJob(
    ...
    if (!oci_execute($stmt, OCI_NO_AUTO_COMMIT)) {
        $e = oci_error($stmt);
        oci_rollback($con);
        responseError("Unable to create declaration job : " . $e['message']);
    }
...
function startDeclarationWorker(int $jobId, string $financialYear): void
{
    $worker = __DIR__ . "/../jobs/processDeclarationDownload.php";

    if (!file_exists($worker)) {
        responseError("Worker not found.");
    }

    ensureDirectoryExists(ITR_JOB_LOG_PATH);

    $logFile = ITR_JOB_LOG_PATH . "/declaration_worker.log";
