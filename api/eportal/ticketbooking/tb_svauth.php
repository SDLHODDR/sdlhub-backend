...
if($data['authForm']==true)
{
    ...
    if($data['flag']=='R')
	{
        ...
        executeQry("update epplive.bcs_trvltkt_request set STATUS='R' , auth_by='".$empCode."', auth_on=sysdate where ID = " . $userTask["TRAN_CODE"] . " ");
        ...
        $individual_taks_dets = singRec("select a.* , 'Ticket from ' || a.trvl_from_loc || ' To ' || a.trvl_to_loc || ' Dated ' || a.trvl_date as newtaskdets from epplive.bcs_trvltkt_request a where id= '".$userTask['TRAN_CODE']."'");

        $sitecode = singRec("select PAY_SITE from epplive.bcs_employee where emp_code='".$individual_taks_dets['EMP_CODE']."'");

        $task_id=executeQry("insert into epplive.bcs_user_tasks  
          ...
          executeQry("update epplive.bcs_trvltkt_request set STATUS='A' , auth_by='".$empCode."', auth_on=sysdate where id= '".$userTask['TRAN_CODE']."' ");
