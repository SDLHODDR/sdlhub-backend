...
try {
    ...
    foreach ($data['ROWS'] as $kOrg => $vlOrg) {
        ...
        if($orgres['EMP_LEVEL']=='15'){
            $geo_det = singRec("SELECT
			    ...
			    FROM HR_DIVISIONS WHERE DIVSN_ID = '" . $vlOrg['GEO_ID'] . "' ");
            ...
            $geo_det = singRec("SELECT
                ...
                WHERE GEO_ID = '" . $vlOrg['GEO_ID'] . "' 
                AND DIVSN_ID = '".$data['DIVSN_ID']."' 
                AND '".$vlOrg['EFFEC_FROM']."'
                BETWEEN EFFEC_FROM AND EFFEC_TO");
        }

        if($data['ID'] == '' && $vlOrg['ID'] == ''){
            $newId = executeQry("INSERT INTO HR_ORGANOGRAM_LOC(ID, ORG_ID, GEO_ID, GEO_DESC, LOC_LABEL, EFFEC_FROM, EFFEC_TO, EMP_LEVEL, STATUS) VALUES('',
                '" . $data['ORGANOGRAM_ID'] . "',
                '" . trim($vlOrg['GEO_ID']) . "',
                '" . trim($geo_det['GEO_DESC']) . "',
                '" . trim($geo_det['GEO_LABEL']) . "',
                '" . trim($vlOrg['EFFEC_FROM']) . "',
                '" . trim($vlOrg['EFFEC_TO']) . "',
                '" . trim($data['EMP_LEVEL']) . "',
                'A'				
            ...
            executeQry("UPDATE HR_ORGANOGRAM_LOC SET
                ...
                EFFEC_FROM='" . trim($vlOrg['EFFEC_FROM']) . "',
                EFFEC_TO='" . trim($vlOrg['EFFEC_TO']) . "',
                EMP_LEVEL='" . trim($data['EMP_LEVEL']) . "',
                STATUS = 'A'
                where ID = '" . $vlOrg['ID'] . "'"); //LOC_ID_hidden
